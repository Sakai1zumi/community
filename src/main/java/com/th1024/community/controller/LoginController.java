...
public class LoginController implements CommunityConstant {
    ...
    public void getKaptcha(HttpServletResponse response/*, HttpSession session*/) {
        ...
        redisTemplate.opsForValue().set(kaptchaKey, text, 60, TimeUnit.SECONDS);// 存入Redis中并设置60秒后过期
    ...
    public String login(String username, String password, String code, boolean isRemembered,
                        ...
                        @CookieValue("kaptchaOwner") String kaptchaOwner /*从cookie中获取验证码归属的字符串*/) {
        // 检查验证码
        //String kaptcha = (String) session.getAttribute("kaptcha");
        String kaptcha = null;
        if (StringUtils.isNotBlank(kaptchaOwner)) { // 判断参数是否为空
            String kaptchaKey = RedisKeyUtil.getKaptchaKey(kaptchaOwner);
    ...
    public void forget(String yourEmail, String verifycode, Model model) {
